...
**Consumo por cliente (interruptor FUTURO):** para que el bot / mantenimiento / itinerario **usen** la plantilla del cliente en vez de la global, un helper `getPlantilla(cliente, tipo)` = `tareas_config/{cliente|tipo}` ?? `tareas_config/{tipo}`. **No se enciende ahora** porque con un solo cliente (RIPLEY) cuya plantilla == la global, no cambia nada y solo mete riesgo; el valor aparece con el **2.º cliente** con actividades distintas. Se activa con su propio Council + prueba en vivo.
