...
"Manta de Observaciones" — sábana de observaciones de mantenimiento de **Ripley** (Roof Tops, 8 tiendas). Reemplaza el Excel manual `MANTA GENERAL - RIPLEY.xlsx`. Construida sobre el scaffold de `proveedores.html` (auth Google/`usuarios`, panel de accesos, footer/toast, responsive). **Estado**: activa en develop y **producción** (merge a main 2026-06-20). **Parte B (bot WhatsApp)**: código completo (Fases 1-5) y desplegado, pero **inerte en producción** hasta cablear Meta (número + env vars); backend en **Vercel Functions**, no Firebase, para no requerir Blaze.
