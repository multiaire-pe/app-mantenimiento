...
"Manta de Observaciones" — sábana de observaciones de mantenimiento de **Ripley** (Roof Tops, 8 tiendas). Reemplaza el Excel manual `MANTA GENERAL - RIPLEY.xlsx`. Construida sobre el scaffold de `proveedores.html` (auth Google/`usuarios`, panel de accesos, footer/toast, responsive). **Estado**: activa en develop. **Parte B pendiente**: bot de WhatsApp que alimentará la manta (backend acordado en **Vercel Functions**, no Firebase, para no requerir Blaze).
...
- **Maestro `manta_equipos`** (tienda → equipos, sembrado del Excel) alimenta los **desplegables dependientes** tienda→equipo (`fillTiendaSelect`/`fillEquipoSelect`/`onObsTiendaChange`) y el export. Las 8 tiendas: PRIMAVERA, JOCKEY PLAZA, SANTA ANITA, MIRAFLORES, PLAZA NORTE, CHORRILLOS, MEGA PLAZA, SAN MIGUEL. Equipos tipo "AA Roof Top NN" (numeración variable; Mega Plaza no correlativa).
...
- **Card** en `index.html` (entre Proveedores y Configuración). `observaciones` añadida a `ALL_APPS` de `proveedores.html` y `comprobantes.html` (los `ALL_APPS` del resto de apps están desactualizados, estado preexistente). Backup en `configuracion.html` (las 3 colecciones `manta_*`).
...
- **Fase 5 (pendiente)** — foto desde WhatsApp (descarga vía Graph API + `manta_observaciones_fotos` + `tieneFoto`) + **aviso a supervisores** (plantilla "utility" de Meta + flag "recibe avisos") + deploy + setup Meta + prueba end-to-end.
...
| `manta_observaciones` | Manta de observaciones de mantenimiento Ripley (`observaciones.html`). **Log**: 1 doc = 1 observación (un equipo tiene varias). Campos: `tienda`, `equipo`, `observacion`, `estado` (`PENDIENTE`/`EN_PROCESO`/`OK`), `fecha` (YYYY-MM-DD), `tieneFoto`, `tecnicoId` (opcional → `maestros_personal`), `origen` (`APP`/`WHATSAPP`), `registradoPor`, `createdAt/By`, `updatedAt/By`. ADMIN/SUPER_ADMIN editan; SUPERVISOR solo lectura |
